...
find_for_home() {
    local user_home="$1" out_file="$2" root
    ...
    for root in "$user_home/.nvm" "$user_home/.npm-global" "$user_home/.local"; do
        [[ -d "$root" ]] && find "$root" -maxdepth 8 -name "claude" \( -type f -o -type l \) 2>/dev/null >> "$out_file" || true
    done
    ...
    find "$user_home" -maxdepth 2 \( \
        ...
        \) -type d 2>/dev/null | while IFS= read -r root; do
        [[ -d "$root/extensions" ]] || continue
        find "$root/extensions" -maxdepth 1 -name "anthropic.claude-code*" -type d 2>/dev/null >> "$out_file" || true
    done
}

find_targets() {
    local out_file="$1" user_home
    : > "$out_file"
    ...
    shopt -s nullglob
    for user_home in "$SEARCH_BASE"/*/; do find_for_home "${user_home%/}" "$out_file"; done
    shopt -u nullglob
    sort -u "$out_file" -o "$out_file"
}

print_list() {
    local list_file="$1" count item
    count=$(wc -l < "$list_file" 2>/dev/null || echo 0)

    if [[ "$count" -eq 0 ]]; then
        echo "RESULT: found 0 item(s)"
        echo "No Claude Code files or directories found."
        return 0
    fi

    echo "RESULT: found $count item(s)"
    echo "Found $count item(s):"
    while IFS= read -r item; do
        if [[ -d "$item" && ! -L "$item" ]]; then echo "  [dir]  $item"
        elif [[ -L "$item" ]]; then echo "  [link] $item -> $(readlink "$item" 2>/dev/null || true)"
        elif [[ -f "$item" ]]; then echo "  [file] $item"
        else echo "  [gone] $item"
        fi
    done < "$list_file"
}
...
perform_deletion() {
    local list_file="$1" deleted=0 failed=0 skipped=0 item target
    ...
    while IFS= read -r item; do
        ...
        if [[ -L "$item" ]]; then
            ...
            if rm -f "$item"; then echo "  Deleted link: $item"; (( deleted++ )) || true
            else echo "  FAILED link:  $item"; (( failed++ )) || true
            fi

            if [[ -n "${target:-}" && -e "$target" ]]; then
                if rm -rf "$target"; then echo "  Deleted target: $target"; (( deleted++ )) || true
                else echo "  FAILED target:  $target"; (( failed++ )) || true
                fi
            fi
        elif [[ -d "$item" ]]; then
            if rm -rf "$item"; then echo "  Deleted dir:  $item"; (( deleted++ )) || true
            else echo "  FAILED dir:   $item"; (( failed++ )) || true
            fi
        elif [[ -f "$item" ]]; then
            if rm -f "$item"; then echo "  Deleted file: $item"; (( deleted++ )) || true
            else echo "  FAILED file:  $item"; (( failed++ )) || true
            fi
        else
            echo "  Skipped missing item: $item"; (( skipped++ )) || true
        fi
    done < "$list_file"

    echo "Deletion complete. Deleted: $deleted  Failed: $failed  Skipped: $skipped"
    if [[ "$failed" -eq 0 ]]; then
        echo "RESULT: deletion succeeded; deleted=$deleted failed=$failed skipped=$skipped"
    else
        echo "RESULT: deletion failed; deleted=$deleted failed=$failed skipped=$skipped"
    fi
    [[ "$failed" -eq 0 ]]
}
...
function Add-OutputLine {
    param([string]$Message, [switch]$Status, [switch]$Log)
    $line = if ($Status) { "[claude-removal-wsl-inline-delete-all] $Message" } else { $Message }
    [void]$Script:OutputBuffer.Add($line)

    $logPath = [Environment]::GetEnvironmentVariable('CLAUDE_REMOVAL_WSL_LOG')
    if ($Log -and -not [string]::IsNullOrWhiteSpace($logPath)) {
        try { [IO.File]::AppendAllText($logPath, $line + [Environment]::NewLine, [Text.Encoding]::UTF8) }
        catch { [Console]::Error.WriteLine("[claude-removal-wsl-inline-delete-all] Failed to write log: $($_.Exception.Message)") }
    }
}

function Write-Status { param([string]$Message) Add-OutputLine $Message -Status -Log }
function Write-ChildOutput { param([string]$Message) Add-OutputLine $Message -Log }
function Write-ParentStatus { param([string]$Message) Add-OutputLine $Message -Status }
function Write-ParentOutput { param([string]$Message) Add-OutputLine $Message }
...
try {
    ...
    if ($runningAsSystem) {
        ...
        Write-ResultJson -Result $result -ExitCode $systemExitCode -RunningAsSystem $runningAsSystem
        exit $systemExitCode
    }
    ...
    Write-ResultJson -Result $result -ExitCode $exitCode -RunningAsSystem $runningAsSystem
    exit $exitCode
}
catch {
    Write-Status "ERROR: $($_.Exception.Message)"
    Write-ResultJson -Result 'error' -ExitCode 1 -RunningAsSystem $runningAsSystem -ErrorMessage $_.Exception.Message
    exit 1
}
