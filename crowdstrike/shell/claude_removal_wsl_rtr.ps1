# CrowdStrike RTR script for removing Claude Code from running WSL distributions.
#
# This file is self-contained for RTR deployment. It runs on the Windows host,
# embeds the Linux removal logic below, stages that embedded Bash script inside
# each running WSL distribution, and executes it as Linux root.

[CmdletBinding()]
param(
    # Delete: remove matching files. List: inventory matching files only.
    [ValidateSet('List', 'Delete')]
    [string]$Mode = 'Delete',

    # Optional single WSL distribution name. If omitted, all running distros are targeted.
    [string]$Distro,

    # Search scope inside Linux. /home covers normal WSL user homes.
    [string]$SearchPath = '/home',

    # Keep the staged Bash script in /tmp for troubleshooting.
...
$EmbeddedRemovalScriptLines = @(
    ...
    '',
    'Finds and optionally removes Claude Code executable files, libraries, and editor extensions.',
    '',
    'Options:',
    '  -l, --list            Generate list only',
    '  -d, --delete          Generate list and delete matches',
    '  -o, --output FILE     Write list to FILE',
    '  -p, --path PATH       Search PATH instead of /home',
    ...
    '        done',
    '    else',
    '        echo "Searching for Claude Code artifacts in: $SEARCH_BASE"',
    '        find_for_home "$SEARCH_BASE" "$out_file"',
    '    fi',
    '',
    ...
    '        -o|--output)',
    '            OUTPUT_FILE="${2:-}"',
    '            [[ -n "$OUTPUT_FILE" ]] || { echo "Error: --output requires a file." >&2; exit 1; }',
    '            shift 2',
    '            ;;',
    '        -p|--path)',
    '            SEARCH_BASE="${2:-}"',
    '            [[ -n "$SEARCH_BASE" ]] || { echo "Error: --path requires a directory." >&2; exit 1; }',
    '            shift 2',
    '            ;;',
    '        -h|--help)',
    '            usage',
...
function Write-Status {
    ...
    Write-Output "[claude-removal-wsl] $Message"
}
...
function Normalize-WslOutputLine {
    ...
    # wsl.exe output can include null bytes when captured by Windows PowerShell.
    # It can also mark the default distro with an asterisk in non-quiet listings.
...
function Invoke-SelfInUserSession {
    ...
    try {
        ...
        $scriptPath = Join-Path -Path $env:ProgramData -ChildPath "claude_removal_wsl_rtr_$([Guid]::NewGuid().ToString('N')).ps1"
        if (-not [string]::IsNullOrWhiteSpace($Script:CurrentScriptPath) -and [IO.File]::Exists($Script:CurrentScriptPath)) {
        ...
        $logPath = Join-Path -Path $env:ProgramData -ChildPath "claude_removal_wsl_session_${SessionId}_$([Guid]::NewGuid().ToString('N')).log"
        ...
        $argumentText = ($ArgumentList | ForEach-Object { ConvertTo-PowerShellLiteral -Value $_ }) -join ' '
        $childCommand = "& $scriptLiteral $argumentText *> $logLiteral"
...
function Invoke-SelfForLoggedOnUsersFromSystem {
    $argumentList = @('-Mode', $Mode, '-SearchPath', $SearchPath)
    if ($Distro) { $argumentList += @('-Distro', $Distro) }
    if ($KeepRemoteScript) { $argumentList += '-KeepRemoteScript' }
    ...
    foreach ($sessionId in $sessionIds) {
        try {
            ...
            $exitCode = Invoke-SelfInUserSession -SessionId $sessionId -ArgumentList $argumentList
            if ($exitCode -ne 0) {
...
if ($Distro) {
    if ($runningDistros -notcontains $Distro) {
        throw "Requested distro '$Distro' is not currently running. Running distros: $($runningDistros -join ', ')"
    }
    $targetDistros = @($Distro)
}
else {
    $targetDistros = $runningDistros
}

$remoteScriptPath = "/tmp/claude_removal_rtr_$([Guid]::NewGuid().ToString('N')).sh"
$overallFailures = 0

Write-Status "Mode: $Mode"
Write-Status "Search path inside WSL: $SearchPath"
Write-Status "Target running distro(s): $($targetDistros -join ', ')"

foreach ($target in $targetDistros) {
    Write-Status "Processing distro: $target"

    try {
        # Stage the embedded Bash through stdin so the RTR script has no
        # dependency on local repository paths or companion files.
        ...
        if ($Mode -eq 'Delete') {
            $runArgs = @('bash', $remoteScriptPath, '--delete', '--path', $SearchPath)
        }
        else {
            $runArgs = @('bash', $remoteScriptPath, '--list', '--path', $SearchPath)
        }

        $runResult = Invoke-WslRootCommand -DistroName $target -CommandArguments $runArgs
